    ...
    try:
        ...
        for rec in response['Reservations']:
            for instance in rec['Instances']:
                ...
                if compute_type != 't2.micro':
                    logging.warning(f'Current instance --> {instance["InstanceId"]} is outside limit of allowed compute capacity, hence terminating')    
                    stop_ec2(instance['InstanceId'])    
                logging.info(f'Current instance --> {instance["InstanceId"]} is within limit of allowed compute capacity')    
        #logging.info(f'Details of ec2 instance id --> {rec}')
...
def stop_ec2(instance_id):
    try:
        ...
        logging.error(f'issue while stopping ec2 --> {instance_id}'.format(e))
...
if __name__=='__main__':
    ...
    parser = argparse.ArgumentParser(description= 'use help switch to know more')
    parser.add_argument(
                          '--asgname',
                          dest='AutoScalingGroupName',
                          type=str,
                          help='AutoScalingGroupName',
                          required=True
                        ) 
    ...
    parser.add_argument(
                        ...
                        dest="DesiredCapacity",
                        type=int,
                        help="desired number of compute resources",
                        required=True
                        )
    args = parser.parse_args()
    scale_in_out(**vars(args))
